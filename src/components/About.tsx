      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 relative z-10">
        ...
        <div className="mb-16">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="font-mono text-xs text-pink-400 uppercase tracking-widest">About the Expert</span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="lg:col-span-4 p-6 rounded-2xl bg-gradient-to-b from-[#130E26] to-[#080412] border border-white/[0.06] backdrop-blur-md flex flex-col justify-between hover:border-purple-500/20 transition-colors duration-300 relative overflow-hidden group">
            {/* Ambient visual overlay representing cybernetic node matrix */}
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,rgba(236,72,153,0.06),transparent_60%)] pointer-events-none" />
            ...
            {/* Resume Callout Info */}
            <div className="p-4 rounded-xl bg-pink-500/10 border border-pink-500/20 relative z-10">
              <span className="font-mono text-[10px] text-pink-400 uppercase tracking-wider font-bold">Primary Core</span>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mt-12">
          {personalInfo.stats.map((stat, idx) => (
            <div
              key={idx}
              className="p-6 rounded-2xl bg-white/[0.02] border border-white/[0.05] hover:border-pink-500/15 hover:bg-white/[0.03] transition-all duration-300 flex flex-col justify-between"
              id={`stat-card-${idx}`}
            >
              <span className="font-sans text-xs text-[#9A9AA5] tracking-wide leading-relaxed">
                {stat.label}
              </span>
              <span className="font-serif italic text-3xl sm:text-4xl text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-pink-400 to-rose-300 mt-4 block font-bold">
                {stat.value}
              </span>
            </div>
          ))}
