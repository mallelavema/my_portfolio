      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 relative z-10">
        ...
        <div className="mb-16">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="font-mono text-xs text-pink-400 uppercase tracking-widest">Chronology</span>
          </div>
        ...
        <div className="relative border-l border-white/[0.06] ml-4 md:ml-8 pl-6 md:pl-10 space-y-12">
          ...
          <div className="absolute top-0 bottom-0 left-0 w-px bg-gradient-to-b from-pink-400 via-purple-500 to-transparent pointer-events-none" />
          ...
          {timelineData.map((item, idx) => (
            ...
            >
              ...
              <div className="absolute -left-[31px] md:-left-[47px] top-1.5 w-6 h-6 rounded-full bg-[#0A0A0B] border border-white/[0.12] flex items-center justify-center group-hover:border-pink-400 group-hover:shadow-lg group-hover:shadow-pink-400/25 transition-all duration-300 z-10">
                {getIcon(item.iconName)}
              </div>

              {/* Glowing vertical node bridge */}
              <div className="absolute -left-[31px] md:-left-[47px] top-7 bottom-0 w-px bg-white/[0.08] group-hover:bg-pink-400/30 transition-colors duration-300" />

              {/* Main Timeline Item Card */}
              <div className="p-6 sm:p-8 rounded-2xl bg-white/[0.02] border border-white/[0.05] group-hover:border-white/[0.12] group-hover:bg-[#0c081f]/40 backdrop-blur-md transition-all duration-300 relative overflow-hidden">
                ...
                {/* Title and Company */}
                <h3 className="font-sans font-black text-xl text-white group-hover:text-pink-300 transition-colors duration-200">
                  {item.title}
                </h3>
                <span className="font-sans text-purple-300 font-medium text-sm block mt-1.5">
                ...
                {item.bullets && (
                  <ul className="mt-5 space-y-3 pl-1">
                    {item.bullets.map((bullet, bulletIdx) => (
                      ...
                      >
                        <span className="w-1.5 h-1.5 rounded-full bg-pink-400 mt-2 shrink-0 group-hover:scale-110 transition-transform" />
                        <span>{bullet}</span>
                      </li>
                    ))}
                  </ul>
